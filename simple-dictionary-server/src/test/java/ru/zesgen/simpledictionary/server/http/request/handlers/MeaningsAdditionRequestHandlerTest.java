...
class MeaningsAdditionRequestHandlerTest {
    ...
    @InjectMocks
    MeaningsAdditionRequestHandler handlerUnderTest;

    @BeforeEach
    void setUp() {
        lenient().when(mockedRequest.getHttpMethod()).thenReturn(MeaningsAdditionRequestHandler.METHOD_TO_HANDLE);
        lenient().when(mockedRequest.getUriPath()).thenReturn(MeaningsAdditionRequestHandler.URI_PATH_TO_HANDLE);
        ...
        lenient().when(mockedDictionary.get(anyString())).thenReturn(meaningsStub);
    ...
    void constructor_shouldExecuteCorrectly() {
        ...
        assertThat(handlerUnderTest.getHttpMethodToHandle()).isSameAs(MeaningsAdditionRequestHandler.METHOD_TO_HANDLE);
        assertThat(handlerUnderTest.getUriPathsToHandle()).isSameAs(MeaningsAdditionRequestHandler.PATHS_TO_HANDLE);
    }

    @Test
    void process_shouldReturnTrueAndSetResponseFieldsCorrectly_withSuitableAndGoodRequestFinishedWithAddition() {
        ...
        verify(mockedResponse).setTextBody(MeaningsAdditionRequestHandler.MESSAGE_IF_ADDITION_IS_SUCCESSFUL);
        ...
        verify(mockedDictionary).add(WORD_STUB, meaningsStub);
